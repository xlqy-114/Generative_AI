...
class MainApp:
    def __init__(self, root):
        ...
        # title
        ttk.Label(self.main_menu,
                  text="Financial Auto Analysis",
                  font=(None,24,'bold')).place(relx=0.25, rely=0.4, anchor='w')

        # buttons
        btn_fetch    = ttk.Button(self.main_menu, text="Fetch PDFs",
                                  command=self.show_fetch,
                                  bootstyle="outline-info")
        btn_analysis = ttk.Button(self.main_menu, text="Analysis",
                                  command=self.show_analysis,
                                  bootstyle="outline-primary")
        btn_settings = ttk.Button(self.main_menu, text="Settings",
        ...
        # pages
        self.analysis = AnalysisFrame(self.feature_container, go_back=self.show_main_menu)
        self.fetch    = FetchFrame(self.feature_container,    go_back=self.show_main_menu)
